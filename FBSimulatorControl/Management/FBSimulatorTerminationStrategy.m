...
- (NSArray *)killSimulators:(NSArray *)simulators withError:(NSError **)error
{
  ...
  [self.logger.debug log:@"Killing %@", [FBCollectionDescriptions oneLineDescriptionFromArray:simulators atKeyPath:@"shortDescription"]];
  for (FBSimulator *simulator in simulators) {
    ...
    if (!simulatorProcess) {
      [self.logger.debug log:@"Will not kill %@ as it is not launched", simulator.shortDescription];
      continue;
    }
    ...
    [self.logger.info log:@"Killed & Shutdown %@", simulator];
  }
  return simulators;
}
...
- (BOOL)killProcess:(FBProcessInfo *)process error:(NSError **)error
{
  ...
  [self.logger.debug log:@"Killing %@", process.shortDescription];
  ...
  [self.logger.debug log:@"Killed %@", process.shortDescription];
