...
{
  [self.logger log:@"%@Did Start => %@", self.prefix, launchInfo.shortDescription];
}

- (void)didTerminate:(BOOL)expected
{
  [self.logger log:@"%@Did Terminate => Expected %d", self.prefix, expected];
}
...
{
  [self.logger log:@"%@Agent Did Launch => %@", self.prefix, agentProcess.shortDescription];
}

- (void)agentDidTerminate:(FBProcessInfo *)agentProcess expected:(BOOL)expected
{
  [self.logger log:@"%@Agent Did Terminate => Expected %d %@ ", self.prefix, expected, agentProcess.shortDescription];
}
...
{
  [self.logger log:@"%@Application Did Launch => %@", self.prefix, applicationProcess.shortDescription];
}

- (void)applicationDidTerminate:(FBProcessInfo *)applicationProcess expected:(BOOL)expected
{
  [self.logger log:@"%@Application Did Terminate => Expected %d %@", self.prefix, expected, applicationProcess.shortDescription];
}

- (void)diagnosticInformationAvailable:(NSString *)name process:(FBProcessInfo *)process value:(id<NSCopying, NSCoding>)value
{
  [self.logger log:@"%@Diagnostic Information Available => %@", self.prefix, name];
}

- (void)didChangeState:(FBSimulatorState)state
{
  [self.logger log:@"%@Did Change State => %@", self.prefix, [FBSimulator stateStringFromSimulatorState:state]];
}
